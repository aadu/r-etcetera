...
#' @param n Optional, number of individual in each group, if left blank, the
#' group will be split evenly into \code{k} groups
#' @param compare Cohort label upon which to check if groups have been split
#' in equivalent portions (default = 'cohortcurrent')
#' @param path Optional directory path for which to store the group files
#' @return list of \code{k} data frames for each group
#' @export
equal_groups = function(x, k, n=NULL, compare='cohort', path=NULL){
  if(is.null(n))
    n = floor(nrow(x) / k)
  ...
  while(!pass){
    ...
    groups <- Map(
      cbind, groups, sapply(k, function(x)(paste0("group", x))))
  ...
  if(!is.null(path))
    lapply(k,function(x)(write.csv(
      data[[x]], file=paste0(path, "version-", LETTERS[x], ".csv"),
      row.names=FALSE, na="")))
